...
- Abort causes are distinguished (user / turn deadline / model-stream idle stall). An idle stall with no emitted step output retries via the shared bounded model-retry pool (sized by the `modelRetries` setting, default 2, range 0–10; reported as `maxRetries` in events), salvaging the conversation to the last completed step; an exhausted stall pauses the turn (status `failed`) preserving the active goal and exposing a one-key R resume interactively. An attempt that ignores cancellation past a grace window is forcibly settled at turn level: owned resources close exactly once (bounded), the attempt's callbacks are permanently quarantined so late output cannot mutate the finished turn, and the turn resolves `aborted` with a truthful teardown report.
- Completion is evidence-gated, not text-gated: a voluntary final is rejected while this turn's `writeTasks` list has pending/in-progress items, post-mutation validation is missing/stale/failed (implement/fix/test intents), asks re-derived from the exact request remain open, a fix-intent goal lacks its red→green pair (pre-mutation failing repro + same-check green, or a structured waiver), or the independent verification slice rejected it. Ask/red-waiver updates are structured-only (`writeTasks` echoes validated against real events; prose can never close evidence), waived scope surfaces in the final synthesis, and one blind fresh-context verify slice runs before a mutating goal's first accepted final (skipped for `trivial` shapes; proportionality keeps the post-mutation-validation floor for every shape). Known test/build commands count automatically; custom checks use `shell` with `purpose=validation` so real process results become structured evidence. Rejected finals trigger autonomous goal continuation inside the same logical turn and budgets; a logical-goal supervisor (`runAgentGoal`) spans physical turns, so a step/tool budget boundary (including `tool-calls` finishes) automatically starts a fresh continuation turn against the preserved conversation while measurable progress continues and the goal deadline remains. Every supervisor boundary appends to a durable goal ledger in the session JSONL (crash-safe frontier; resumable on session resume), and headless `--until-done` relaunches transient model failures against it with a no-progress poison guard. It stops only for structured completion, hard blockers, user cancellation, the goal deadline, or one corrective physical turn without measurable outcome progress — never reporting incomplete work as `complete`. Old workspace task files never block unrelated turns; carried evidence hydrates continuation turns via seq baselines so validation debt survives the boundary.
