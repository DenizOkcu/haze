...
- Abort causes are distinguished (user / turn deadline / model-stream idle stall). An idle stall with no emitted step output retries via the shared bounded model-retry pool, salvaging the conversation to the last completed step; an exhausted stall pauses the turn (status `failed`) preserving the active goal and exposing a one-key R resume interactively.
- Completion is evidence-gated, not text-gated: a voluntary final is rejected while this turn's `writeTasks` list has pending/in-progress items or post-mutation validation is missing/stale/failed (implement/fix/test intents). Rejected finals trigger autonomous goal continuation inside the same logical turn and budgets; when the global budget or a one-nudge no-progress guard stops continuation, the turn pauses as `failed` with an `incomplete-goal` resume (R key interactively), never as `complete`. Old workspace task files never block unrelated turns.
