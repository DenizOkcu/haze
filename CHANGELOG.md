...
- Autonomous goal continuation: a voluntary final message is no longer accepted as a completed turn while structured evidence shows unfinished work — pending/in-progress items in a task list the model declared this turn via `writeTasks`, or missing/stale/failed validation after edits on implement/fix/test requests. haze rejects the stop and continues the same logical turn (same conversation, budgets, and work state) with a synthetic control that requires resuming the next concrete task rather than summarizing again. Continuation slices are clamped to the remaining global step/tool/deadline budgets and never reset them; a progress guard allows exactly one corrective nudge without measurable progress. When continuation cannot proceed (budget exhausted or the guard trips), the turn pauses as `failed` with an explicit "unfinished task paused" message, a one-key `R` resume against the preserved conversation, and an `incomplete-goal` resume descriptor carrying safe metadata only. Headless `--output json`/`stream-json` results expose the gating evidence (task counts, validation outcome, `recoveryUsed.goal`) and exit non-zero. Prose never counts as completion evidence, and stale workspace task files from earlier turns never block unrelated turns.
- Estimated USD cost tracking (F-12): provider presets now carry per-model pricing (USD per 1M input/output/cache tokens, from the same models.dev catalog as the context-window limits) and the wizard writes it into `modelLimits` alongside the limits. When the active model has pricing, the status bar appends a running `$` estimate, the debug token panel breaks it out, and the headless `--output json`/`stream-json` `usage` envelope gains an optional `costUsd` field. Amounts are estimates from provider-reported token counts; subscription-included models (e.g. the z.ai coding plan, Codex SKUs) carry no pricing and show no cost.
