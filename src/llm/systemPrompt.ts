...
export function buildSystemPrompt(contextFiles: ContextFile[] = [], session?: PromptSession, options: {lspAvailable?: boolean; mcpAvailable?: boolean; model?: {provider: string; name: string}; availableTools?: ReadonlySet<string>} = {}) {
  ...
  const lspToolRule = options.lspAvailable
    ? '- When LSP tools are available for a file type, prefer them for semantic code navigation. Use lspWorkspaceSymbols to discover an unknown defining file, then lspFindSymbol with a file-scoped hierarchical name path for targeted children, body, or signature retrieval. Use position-based definition/type/implementation queries only at real symbol occurrences. Prefer lspReferences for impact context, lspRenameSymbol for semantic renames, and lspSafeDeleteSymbol for checked removals. Successful file-mutation results already include automatic LSP diagnostics for supported changed files; do not immediately repeat them with lspDiagnostics. lspDiagnosticsForSymbol is a focused intermediate check; relevant tests/typechecks remain authoritative. Fall back to grep/readFile and textual edits when LSP is unavailable or unsuitable.\n'
    : '';
...
- editFile performs unique replacements. If an edit fails, read that exact file again before retrying; use replaceLines when current line numbers are safer. For repeated cross-file text changes, use replaceInFiles with a dry run and current occurrence IDs or an expected-count guard.
