...
- `goalCheckpoint.ts` is the leaf module for bounded continuation checkpoints: the `IncompleteGoalResume` payload on `TurnResult.resume`, the supervisor-level `GoalCheckpoint`, the cumulative progress signature, the request hash binding (`hashRequest`), `checkpointFromGoalFrontier` (rebuild a checkpoint from a durable ledger frontier, carrying red→green evidence so a crash resume matches in-process continuation), and the single checkpoint construction site (`buildIncompleteGoalResume`). Safe metadata only (reasons, counts, enums, and red-evidence command strings without output bodies) — never raw content or credentials. Red→green state is goal-scoped and rides the checkpoint + `seedCarriedGoalEvidence` across physical turns. It must stay import-cycle-free (imports core policy types only) because both `streaming.ts` and `goalSupervisor.ts` depend on it.
