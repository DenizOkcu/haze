...
- `streamLoop.ts` drives the `ToolLoopAgent` loop: loop state (`AttemptLoopState`, created before the stall guard so stall classification can read what the stalled step emitted), public stream-part dispatch, and post-stream conversation commit. The per-concern handlers live in siblings: `prepareStep.ts` (the per-step request decision tree: scoped context injection, mid-turn compaction, and the ordered model-constraint guards), `assistantSegments.ts` (assistant segment lifecycle: lazy start, finalize with duplicate/lead-in suppression, reset), and `toolPartHandlers.ts` (finished tool-call application to display/events/goal state).
...
- `verifySlice.ts` owns the independent verification slice (P3): `verificationRequired` (proportionality trigger — mutating intent, ≥1 mutation, shape not `trivial`, not yet verified) and `runVerificationSlice` (dispatch the blind `validate`-mode verifier via `runVerifier`, then `applyVerifierVerdict`). One slice per physical turn (`TurnExecutionState.verifySliceUsed`); admission happens in `agentAttempt.ts` only when every other readiness gate already passes and ≥60s turn time remains; the slice never re-arms budgets. A `not-verified` verdict rejects the voluntary final (`verification_rejected`), and the verifier's named gaps ride the continuation prompt; a rejected final clears the stale verdict so the next attempt re-verifies freshly.
- It also owns the multi-lane final sweep (P5): `sweepRequired` (shape `multi-lane`, verification already passed, not yet swept) and `runSweepSlice` (dispatch the blind read-only `inspect` worker via `runSweep`, then `applySweepVerdict`). Advisory by design: findings surface as a system line; only explicitly reported concrete regressions gate (through the verifier-verdict path); a dispatch failure is a no-op, never a blocker. One sweep per logical goal (`goal.sweepDone`); same admission discipline as the verify slice.
...
- `goalCheckpoint.ts` is the leaf module for bounded continuation checkpoints: the `IncompleteGoalResume` payload on `TurnResult.resume`, the supervisor-level `GoalCheckpoint`, the cumulative progress signature, the request hash binding (`hashRequest`), `checkpointFromGoalFrontier` (rebuild a checkpoint from a durable ledger frontier, carrying ask statuses and red/verification evidence so a crash resume matches in-process continuation), and the single checkpoint construction site (`buildIncompleteGoalResume`). Safe metadata only (reasons, counts, enums, ask texts derived from the user request, red-evidence command strings without output bodies) — never raw content or credentials. Ask/shape/red/verified state is goal-scoped and rides the checkpoint + `seedCarriedGoalEvidence` across physical turns. It must stay import-cycle-free (imports core policy types only) because both `streaming.ts` and `goalSupervisor.ts` depend on it.
