...
export function TaskBar({tasks, width, expanded, padding, maxRows}: {tasks: Task[]; width: number; expanded: boolean; padding: number; maxRows?: number}) {
  ...
  return (
    <Box flexDirection="column" flexShrink={0}>
      ...
      <Text><Text color={theme.accent} bold>Tasks</Text>{counts ? <Text color={theme.muted}> ({counts})</Text> : null}{tasks.length > MAX_VISIBLE_TASKS ? <Text color={theme.muted} dimColor> · ctrl+o {expanded ? 'collapse' : 'expand'}</Text> : null}</Text>
      {ordered.map(task => {
        ...
        return (
          <Text key={task.id} wrap="truncate-end">
            ...
            <Text color={task.status === 'completed' ? theme.muted : theme.foreground}>{title}</Text>
          </Text>
        );
      })}
