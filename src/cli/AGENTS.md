...
- `chat/*.ts(x)` contains chat-specific helpers/components extracted from `chat.tsx`: `sessionLifecycle.ts` (session init/continue/resume/new/clear/compact controller), `startupSequence.ts` (startup banner/session-init/skills/update-check/diagnostics sequence + branch polling), `followUpQueue.ts` (queued follow-ups while busy), `busyIndicator.ts` (busy state with the one-second heartbeat), `wizardDispatch.ts` (wizard submit engine composition; the per-mode handler families live in `wizard/` — provider/model, skills, LSP, MCP, session/theme — plus pure `wizard/fieldTransitions.ts` and the `wizard/uiState.ts` reducer that owns selection/drafts/model-discovery state), `chatMetrics.ts` (token/status-bar math), `transcriptPartition.ts` (pure static/dynamic transcript partitioning), and `messages.tsx` (message views). `commands/streaming.ts` is the thin turn facade (`runAgentTurn` + turn types); the attempt phases (setup, stream loop, stall recovery, attempt outcome, forced-settlement lifecycle) live under `commands/streaming/` (see its AGENTS.md), including the authoritative `attemptOutcome.ts` status function.

## UI state rules

Maintainability focus:

- Treat `commands/chat.tsx` as orchestration glue; session and wizard behavior belong in `chat/sessionLifecycle.ts` / `chat/wizardDispatch.ts` (+ `chat/wizard/` handler families). New wizard steps are added as one entry in the `WIZARD_STEPS` table in `wizardFlow.ts` (plus a submit handler in the matching `chat/wizard/*Handlers.ts` when the step mutates anything), not as inline branches in `submit()`.
