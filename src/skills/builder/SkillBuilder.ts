...
export async function createSkillScaffold(name: string, toolName: string, description: string, toolDescription = description) {
...
export async function buildSkill(description: string) {
  ...
  if (!ok) return;
  await createSkillScaffold(name, toolName, description, toolDescription);
