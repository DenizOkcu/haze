...
- `goal` — durable goal-ledger boundary (P1): one append per supervisor `goal_start`/`goal_continue`/`goal_end` carrying `goalId`, exact request + `requestHash`, intent, cycle, mutation/validation counts, progress signature, shape, full ask statuses (`asks`; legacy `openAsks` maps to open asks), and the red→green/verification evidence (`redEvidence`, `redWaiverReason`, `greenSuccessor`, `verified`) so a crash resume matches in-process continuation. The frontier is the last non-terminal entry of the newest goal id (`findGoalLedgerFrontier` / `readGoalLedgerFrontier` / `restoreSessionState().goalFrontier`); a truncated tail line parses as absent. `sessionSlimming.ts` caps only the request text (1024 chars + marker) and never drops frontier entries. The vacuum bounds the audit trail: only the trailing `GOAL_LEDGER_KEEP_PER_GOAL` entries per goal id survive a rewrite (the frontier and terminal entry are always kept; trims are amortized via `GOAL_LEDGER_TRIM_BYTES`). A goal-only session is not resumable on its own.
