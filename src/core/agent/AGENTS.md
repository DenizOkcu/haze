...
- `events.ts` defines structured agent events for sessions/headless/UI. Additive changes are preferred. `goal_notice` carries gate decisions (such as pauses) to headless consumers, where non-assistant messages are otherwise invisible; `goal_continue` additionally carries a human-readable `text`.
