...
- `events.ts` defines structured agent events for sessions/headless/UI. Additive changes are preferred. `goal_notice` carries gate decisions (waiver surfacing, pauses) to headless consumers, where non-assistant messages are otherwise invisible; `goal_continue` additionally carries a human-readable `text`.
- `workState.ts` defines structured work state included in compaction/session snapshots. It records mutation/validation sequence numbers so a validation that predates the latest mutation is `stale` (`deriveValidationOutcome`: passed/failed/stale/absent/not_applicable). A classifier-confirmed shell command (output carries a `validationSummary`) is recorded as validation; direct, unchained execution of a file changed during the active goal also counts as bounded runtime evidence. Arbitrary shell calls remain process work. A successful `writeTasks` result records bounded current-turn task counts in `taskProgress` (counts only, via `taskProgressFromOutput`; malformed output is ignored), and `redWaiver`/`greenSuccessor` declarations (P4). Red→green pair evidence (`redEvidence` captured only from a failing validation observed before the first mutation of a fix intent; `redPairStatus` binds green to the same normalized command, an explicit successor, or a waiver) is goal-scoped and rides checkpoints via `seedCarriedGoalEvidence`. Task evidence is per-turn unless hydrated from a goal checkpoint via `seedCarriedGoalEvidence` (seq baselines + `carriedValidation` keep cross-physical-turn semantics truthful: carried stale/absent keeps demanding validation, carried passed/failed stands until the turn mutates or validates); a stale workspace `tasks.json` from an unrelated turn must never block completion.
