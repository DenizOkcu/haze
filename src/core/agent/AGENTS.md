...
- `completionController.ts` is the pure, unit-testable home for the authoritative turn-completion decision (`decideTerminalStatus`), completion readiness (`assessCompletionReadiness`: ready / pending_tasks / pending_asks / validation_failed / validation_stale / validation_absent_after_mutation / missing_red_evidence / verification_rejected / tool_failure / unresolved_tool_input / aborted), terminal classification (`classifyTerminalOutcome`: goal-complete / recoverable-incomplete / hard-blocked / user-aborted), turn-wide budget exhaustion (`isBudgetExhausted`), the turn-wide `TurnExecutionState`, finish-cause normalization, recovery decision entry points, and the "satisfactory terminal outcome" guard. The CLI `attemptOutcome.ts` adapter keeps the single call site and delegates here; do not duplicate status inference elsewhere. Recovery decisions default to disabled and are enabled incrementally with explicit guards (abort wins, credit once-only, global budget enforced).
...
- `goalPolicy.ts` is the shared home for goal/completion policy: request-intent classification (`classifyRequestIntent` / `isPlanOnlyRequest` — deterministic heuristics, hints not hard authorization; plan-only requests must not lead to source mutations unless the user asks), deterministic ask extraction (`deriveRequestAsks`: 1–7 imperative-clause asks from the exact request, bounded/deduped; extraction failure degrades to no asks, never a deadlock), proportional goal shapes (`classifyGoalShape`: trivial/bounded/multi-lane/debug; `escalateGoalShape` is up-only), session-goal state (`createSessionGoal` / `observeGoalToolEvent` / `formatGoalStatus`, building on `workState`; `createSessionGoal` seeds request-derived asks and a shape for mutating intents), and the model-facing completion/continuation control prompts (`repeatedToolCallPrompt`, `toolLoopBudgetPrompt`, `lengthContinuationPrompt`, `completionRescuePrompt`, `malformedToolCallPrompt`, `goalContinuationPrompt` — names open asks and the structured closure path — `fixDepthPrompt`). Keep prompts small and reusable — one shared helper over near-identical control text in multiple loops; they are one-request nudges, never durable conversation history. Goal status text is shown in UI; keep it short and stable.
...
- `workState.ts` defines structured work state included in compaction/session snapshots. It records mutation/validation sequence numbers so a validation that predates the latest mutation is `stale` (`deriveValidationOutcome`: passed/failed/stale/absent/not_applicable). A classifier-confirmed shell command (output carries a `validationSummary`) is recorded as validation; direct, unchained execution of a file changed during the active goal also counts as bounded runtime evidence. Arbitrary shell calls remain process work. A successful `writeTasks` result records bounded current-turn task counts in `taskProgress` (counts only, via `taskProgressFromOutput`; malformed output is ignored), echoed `askUpdates` (structurally validated by `applyAskUpdate` against real passing validations or changed files — prose alone never closes an ask; waivers require a reason), and `redWaiver`/`greenSuccessor` declarations, plus `goalShape` proposals (applied up-only via `escalateGoalShape`; downward proposals, failed calls, and shapeless goals are inert). Ask state (`asks: open/met/waived`), goal shape, red→green pair evidence (`redEvidence` captured only from a failing validation observed before the first mutation of a fix intent; `redPairStatus` binds green to the same normalized command, an explicit successor, or a waiver), and verifier verdicts (`applyVerifierVerdict`) are goal-scoped and ride checkpoints via `seedCarriedGoalEvidence`. Task evidence is per-turn unless hydrated from a goal checkpoint via `seedCarriedGoalEvidence` (seq baselines + `carriedValidation` keep cross-physical-turn semantics truthful: carried stale/absent keeps demanding validation, carried passed/failed stands until the turn mutates or validates); a stale workspace `tasks.json` from an unrelated turn must never block completion.
