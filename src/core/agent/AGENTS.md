...
- `workState.ts` defines structured work state included in compaction/session snapshots. It records mutation/validation sequence numbers so a validation that predates the latest mutation is `stale` (`deriveValidationOutcome`: passed/failed/stale/absent/not_applicable). A classifier-confirmed shell command (output carries a `validationSummary`) is recorded as validation; direct, unchained execution of a file changed during the active goal also counts as bounded runtime evidence. Arbitrary shell calls remain process work. A successful `writeTasks` result records bounded current-turn task counts in `taskProgress` (counts only, via `taskProgressFromOutput`; malformed output is ignored). Task evidence is per-turn unless hydrated from a goal checkpoint via `seedCarriedGoalEvidence` (seq baselines + `carriedValidation` keep cross-physical-turn semantics truthful: carried stale/absent keeps demanding validation, carried passed/failed stands until the turn mutates or validates); a stale workspace `tasks.json` from an unrelated turn must never block completion.
