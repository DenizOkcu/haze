...
The `status` field is authoritative (driven by the agent's terminal state, not by parsing `result`), and the exit code mirrors it: `0` only for `complete`. A run that ends without a substantive final answer, after an unresolved final tool failure, with declared tasks or required validation still unfinished, with asks from the original request still unmet, or (for fixes) without a captured failing repro that the same check now passes — even if the model wrote a final message — is `failed` even if the provider returned normally; the evidence envelope carries the task counts, validation outcome, ask state, and red-green pair that gated it. Skipped scope is never silent: waived asks and red-evidence waivers surface in the final summary with their reasons. Before a mutating goal first reports done, an independent verifier subagent re-derives the request against the repository in a fresh context and can reject the final with named gaps. Per-turn step/tool budgets are safety boundaries, not completion: haze automatically continues the logical goal in fresh turns while measurable progress remains, so a budget boundary alone never fails a run. `--timeout` bounds the entire logical goal (not each internal turn), and `--until-done` additionally relaunches the goal after transient provider failures (exponential backoff, `goal_resume` events, a three-strike no-progress guard) until it structurally completes or the deadline hits — without granting any extra authority. The result envelope additionally carries a `goal` object (physical turns used, stop reason, total mutations, final validation outcome and task counts) when goal-level continuation was involved, and a bounded `notices` array with the gate decisions the run made (verification verdicts, waived scope, pause reasons) so scripts can see why a goal continued or stopped — the same decisions stream live as `goal_notice` events in `--output stream-json`.
